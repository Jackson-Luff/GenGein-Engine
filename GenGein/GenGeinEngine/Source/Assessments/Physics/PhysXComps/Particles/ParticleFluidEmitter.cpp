...
//constructor
ParticleFluidEmitter::ParticleFluidEmitter(int _maxParticles, PxVec3 _position, PxParticleFluid* _pf, float _releaseDelay, FBXModel* a_model)
{
...
//updateParticle
void ParticleFluidEmitter::Update(const double delta)
{
	//tick the emitter
	m_time += (float)delta;
	...
	if (fd)
	{
		...
		for (unsigned i = 0; i < fd->validParticleRange; ++i, ++flagsIt, ++positionIt,++densityIt)
		{
			if (*flagsIt & PxParticleFlag::eVALID)
			{
				...
				m_model->SetLocalTransform(glm::translate(pos) * glm::scale(glm::vec3(0.12f)));
				m_model->Render();
			}
		}
		// return ownership of the buffers back to the SDK
