...
void AudioHandle::Initialise()
{
	...
	// NOTE: Fix Create2DSound to remove mess
	// Background Piano
	FMOD_RESULT result;
	result = m_pFmodSystem->createSound("Data/Audio/debussy.mp3",
		FMOD_LOOP_NORMAL | FMOD_2D, 0, &m_pPianoSound);
	if (result != FMOD_OK)
	{
	...
	result = m_pFmodSystem->createSound("Data/Audio/fireplace.wav",
		FMOD_LOOP_NORMAL | FMOD_2D, 0, &m_pFireSound);
	if (result != FMOD_OK)
	{
		printf("FMOD error! (%d) %s \n", result, FMOD_ErrorString(result));
		return;
	...
	// Slide on movement
	result = m_pFmodSystem->createSound("Data/Audio/slide.wav",
		FMOD_DEFAULT | FMOD_2D, 0, &m_pSlideSound);
	if (result != FMOD_OK)
	{
		printf("FMOD error! (%d) %s \n", result, FMOD_ErrorString(result));
		return;
	...
	// Click on has moved
	result = m_pFmodSystem->createSound("Data/Audio/click.wav",
		FMOD_DEFAULT | FMOD_2D, 0, &m_pMoveSound);
	if (result != FMOD_OK)
	{
		printf("FMOD error! (%d) %s \n", result, FMOD_ErrorString(result));
		return;
...
bool AudioHandle::Create2DSound(const char* a_dir, FMOD::Sound& a_sound, const uint32_t a_fmodType)
{
	...
	*/
	...
	FMOD_RESULT result;
	result = m_pFmodSystem->createSound(a_dir, a_fmodType | FMOD_2D, 0, (FMOD::Sound**)&a_sound);
	if (result != FMOD_OK)
	{
		printf("FMOD error! (%d) %s \n", result, FMOD_ErrorString(result));
		return false;
...
bool AudioHandle::Create3DSound(const char* a_dir, FMOD::Sound& a_sound, const FMOD_VECTOR a_pos, const uint32_t a_fmodType)
{
	...
	*/
	...
	result = m_pFmodSystem->createSound(a_dir, a_fmodType | FMOD_3D, 0, (FMOD::Sound**)&a_sound);
	if (result != FMOD_OK)
	{
		printf("FMOD error! (%d) %s \n", result, FMOD_ErrorString(result));
		return false;
